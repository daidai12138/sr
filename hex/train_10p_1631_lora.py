...
"""
Training script (optimized) for MUSK + regression head with optional LoRA.

Key optimizations vs your original:
- Fixes typos/bugs (labels_f, stray 'fo', inconsistent losses)
- Adds optional LoRA training for the visual backbone (freeze backbone weights, train LoRA + heads)
- Adds Early Stopping by val Pearson (small-data friendly)
- Saves best checkpoint by val Pearson (you care about correlation)
- Makes sample/pro_index/train_ratio configurable (removes hard-coded P3/898/80)
- Keeps AMP, grad clipping, warmup+cosine, and robust loss support
"""
...
# project modules
from hex.hex_architecture_10p_lora_1631 import CustomModel
from hex.utils_10p_1631 import PatchDataset, seed_torch  # noqa: F401
...
def set_trainable_for_stage(model: torch.nn.Module, stage: int, unfreeze_last_k: int = 2):
    """
    stage 0: train heads only
    stage 1: also unfreeze last K encoder layers (best-effort for beit3-style backbones)
    """
    ...
    # best-effort: model.visual.beit3.encoder.layers[-k:]
    try:
...
def build_param_groups(model, base_lr_backbone: float, base_lr_head: float, weight_decay: float):
    """Backbone vs heads; avoid weight decay for LN/bias/1D params."""
...
class LoRALinear(torch.nn.Module):
    ...
    W_out = W + (alpha/r) * (B @ A), where A: (r, in), B: (out, r)
    """
...
def finetuned_state_dict(model: torch.nn.Module):
    """Only save trainable params/buffers to keep checkpoints smaller."""
    prefixes = set()
    for n, p in model.named_parameters():
        if p.requires_grad:
...
def main():
    ...
    parser.add_argument("--pro-index", type=int, default=898)
    ...
    # losses
    parser.add_argument("--use-robust", action="store_true", help="use robust_loss_pytorch")
    parser.add_argument("--mse-weight", type=float, default=1.0, help="mse weight")
    parser.add_argument("--robust-weight", type=float, default=0.0, help="robust weight")
    parser.add_argument("--pearson-weight", type=float, default=0.1, help="pearson loss weight")
    ...
    # LoRA
    parser.add_argument("--use-lora", action="store_true", help="enable LoRA for visual backbone")
    ...
    parser.add_argument(
        ...
        help="regex over module names to replace nn.Linear with LoRA",
    )
    ...
    num_outputs = 1631  # your current task is scalar regression
    ...
    # ---------------------------
    # Build split from expression table
    # ---------------------------
    sample = args.sample
    pro_index = args.pro_index
    ...
    pro_array = np.load(join(args.data_dir,'nuclear_1631.npy'))
    ...
    print("\n" + "=" * 80)
    print(f"[SAMPLE] {sample} | pro_index={pro_index} | train={len(train_dataset)} val={len(val_dataset)} | img={args.img_size}")
    ...
    # stage 0: heads only (unless LoRA)
    set_trainable_for_stage(model, stage=0, unfreeze_last_k=args.unfreeze_last_k)

    # LoRA injection
    if args.use_lora:
        # Freeze whole backbone first (LoRA wrapper will also freeze its base Linear)
        ...
        if len(replaced) == 0:
            print("[LoRA][WARN] No Linear modules matched. Print model.visual.named_modules() and adjust --lora-target-regex.")
    ...
    def make_optim_and_sched():
        param_groups = build_param_groups(
            model,
            base_lr_backbone=args.lr_backbone,  # for LoRA params when enabled
        ...
        if criterion_ad is not None:
            opt.add_param_group({"params": list(criterion_ad.parameters()), "lr": args.lr_robust, "weight_decay": 0.0, "name": "robust_params"})
    ...
    # logging / ckpt dirs
    # ---------------------------
    run_name = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_{sample}_p{pro_index}"
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        best_val_r = float(ckpt.get("best_val_r", best_val_r))
        best_epoch = int(ckpt.get("best_epoch", best_epoch))
        print(f"[RESUME] {args.resume} | start_epoch={start_epoch} best_val_r={best_val_r:.4f}")
    ...
    for epoch in range(start_epoch, args.epochs):
        # if not LoRA: optional stage switch after warmup
        ...
        pbar = tqdm.tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Train {epoch+1}/{args.epochs}", leave=False)
        ...
        with torch.no_grad():
            pbar = tqdm.tqdm(enumerate(val_loader), total=len(val_loader), desc=f"Val {epoch+1}/{args.epochs}", leave=False)
        ...
        # ---- checkpoint / early stop (by val Pearson) ----
        ...
        if args.early_stop_patience > 0 and patience >= args.early_stop_patience:
            print(f"[EARLY STOP] No improvement in val Pearson for {patience} epochs. Stop.")
            break
