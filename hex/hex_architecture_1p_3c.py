...
class CustomModel(nn.Module):
    def __init__(
        ...
        self.film = nn.Sequential(
            nn.Linear(molan_n+2, 10),
            nn.GELU(),
            nn.Linear(10, 2 * midle_n),
        )
        ...
        self.out_head_2 = nn.Sequential(
            ...
            nn.Linear(midle_n, midle_n),
            #nn.GELU(),
            #nn.Dropout(0.1),
            nn.Linear(midle_n, 128),
            nn.Linear(128, 16),
            nn.Linear(16, molan_n),
        )
    ...
    def forward(
        ...
        # ✅ FiLM 调制（建议 film 用 float32 更稳；这里保持跟 grid_feat dtype 一致也可以）
        coord = torch.tensor([5/9, 5/9], device=point.device).view(1,2).repeat(B,1)
        film_in = torch.cat([point.squeeze(1), coord], dim=-1)  # (B,5)

        gb = self.film(film_in)  # (B, 2*1024)

        gamma, beta = gb.chunk(2, dim=-1)  # (B,1024), (B,1024)

        gamma = gamma[:, None, None, :].expand(B, self.grid_h, self.grid_w, self.midle_n )
        beta  = beta[:, None, None, :].expand(B, self.grid_h, self.grid_w, self.midle_n)

        fused = grid_feat * (1.0 + gamma) + beta  # (B,H,W,1024)
