...
from hex.utils_point_1p_all import *  # PatchDataset, seed_torch, print_network 等
...
def main():
    ...
    #parser.add_argument("--resume", type=str, default="./experiment/checkpoints/<run_name>/checkpoint_best_ft.pth")
    ...
    parser.add_argument("--sample-list", nargs="+", default=["N1","N2", "N3", "N4", "N5", "P1", "P2", "P3","P4", "P5"])
    #parser.add_argument("--sample-list", nargs="+", default=["N2"])
    ...
    parser.add_argument("--resume", type=str, default="./experiment/checkpoints/20260129_192031/checkpoint_best_ft.pth", help="path to checkpoint .pth (optional)")
    ...
    def build_csv(sample_list):
        ...
        for sample_id in sample_list:
            ...
            df["pro_index"] = pro_index + np.arange(len(path_list))
            all_csvs.append(df)
            pro_index += len(path_list)
        return pd.concat(all_csvs).reset_index(drop=True)

    all_csv = build_csv(args.sample_list)
    all_csv = all_csv[all_csv["pro_index"] % 2 == 0].copy()
    all_csv = all_csv.sample(frac=0.5, random_state=args.seed).reset_index(drop=True)
    
    n = len(all_csv)
    n_train = int(n * 0.8)
    train_csvs = all_csv.iloc[:n_train].reset_index(drop=True)
    val_csvs  = all_csv.iloc[n_train:].reset_index(drop=True)
    ...
    # Dataset
    train_dataset = PatchDataset(train_csvs, args.data_dir, molan, transform_train)
    val_dataset = PatchDataset(val_csvs, args.data_dir, molan, transform_val)
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        model.load_state_dict(ckpt.get("ft_model", ckpt), strict=False)
        state = ckpt.get("ft_model", None)
    ...
    stage = 1 if start_epoch >= args.warmup_epochs else 0
    set_trainable_for_stage(model, stage=stage, unfreeze_last_k=args.unfreeze_last_k)
    
    # ✅ 重新按当前 stage 构建 optimizer/scheduler（你的 param_groups 依赖 requires_grad）
    param_groups = build_param_groups(
        model,
        base_lr_backbone=args.lr_backbone,
        base_lr_head=args.lr_head,
    ...
    for epoch in range(start_epoch, args.epochs):
      ...
      f"loss_var={lv_avg:.6f} | loss_tv={ltv_avg:.6f}")
        ...
        "model": model.state_dict(),                 # ✅建议存完整
        "criterion_ad": criterion_ad.state_dict(),   # ✅robust loss 参数
