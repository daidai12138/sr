...
from hex.utils import *  # PatchDataset, seed_torch, print_network 等
...
def main():
    ...
    # 读 molan
    molan_path = join(args.csv_dir, f"molan_{args.molan_n}.csv")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_p:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=torch.cuda.is_available()):
                outputs = model(inputs)  # (B,10,10,molan_n)
        ...
        if is_best:
            ...
            print(f"  [BEST] saved checkpoint_best.pth (val_mse={val_mse:.6f})")

        # per-epoch（可选）
        #torch.save(ckpt, join(ckpt_dir, f"checkpoint_epoch_{epoch+1}.pth"))
